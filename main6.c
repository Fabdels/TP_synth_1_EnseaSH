...

	{
		...
		// Le fils s'occupe d'executer les commandes
	  	...
	  	{	
			...
			Le code ci-dessous permet seulement de
			récupérer le premier argument (avant un espace)
			et le reste des arguments (après un espace), 
			il n'est pas compatible avec execXY (execl, execv, etc..)
			...
			*/
  			...
  			char path[position];
			char args[size_input-position];

  		
			substring(input, path,1,position);


			substring(input, args,position+2,size_input-position);
